...
export const PyramidBracket: React.FC<PyramidBracketProps> = ({ category }) => {
    ...
    const matchesByPhase = phases.reduce((acc, phase) => {
        // Filter out VAI matches (BYEs) from the visual bracket to keep it clean
        const phaseMatches = (category.pyramidMatches || []).filter(m => m.phase === phase && !m.vaiWinner);
    ...
    const MatchCard: React.FC<{ match: any }> = ({ match }) => {
        ...
        return (
            <div className="border-2 border-slate-700/50 rounded-2xl p-5 m-2 bg-slate-800/80 backdrop-blur-md shadow-2xl w-80 relative overflow-hidden group hover:border-blue-500/50 transition-all duration-300">
                ...
                <div className="flex justify-between items-center mb-3">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Encuentro #{match.matchNumber}</p>
    ...
    return (
        <div className="h-screen w-screen bg-slate-950 text-white flex flex-col overflow-hidden relative">
            ...
            <div className="flex-grow overflow-x-auto overflow-y-hidden flex items-center p-12 space-x-16 scrollbar-hide relative z-10">
